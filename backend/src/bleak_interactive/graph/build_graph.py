...
from bleak_interactive.nodes import answer_node, clarify_node, rating_node, structure_questions_node, wait_for_human_node
...
def get_bleak_graph():
    """Build and compile the bleak graph"""
    builder = StateGraph(
        BleakState,
    ...
    builder.add_node("rating_node", create_node_wrapper(rating_node))
    ...
    builder.add_edge("structure_questions_node", END)
    # builder.add_edge("clarify_node", "answer_node")
    # builder.add_edge("answer_node", "rating_node")
    # builder.add_edge("rating_node", END)

    return builder.compile()

def create_answer_graph():
    """Build and compile the answer graph for processing user responses"""
    builder = StateGraph(
        BleakState,
        input=BleakStateInput,
        output=BleakStateOutput
    ...
    # Add nodes for the answer flow
    builder.add_node("answer_node", create_node_wrapper(answer_node))
    builder.add_node("rating_node", create_node_wrapper(rating_node))
    
    # Add edges for answer flow
